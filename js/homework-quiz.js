...
// 在 startQuizSession 函数中找到 apiUrl 定义的部分进行修改
async function startQuizSession() {
    ...
    // 🔍 诊断 1：看看 URL 到底抓到了什么参数
    console.log("=== [诊断] 启动答题 Session ===");
    console.log("URL 参数 id:", chapterId);
    console.log("URL 参数 source:", source);
    console.log("URL 参数 mode:", mode);

    try {
        questionTextEl.textContent = '正在准备题目，请稍候...';
        let apiUrl = '';
        
        // 🔍 诊断 2：检查 source 判断分支
        if (source === 'homework') {
            console.log("✅ 检测到作业模式，重定向至 get-homework-data");
            apiUrl = `/.netlify/functions/get-homework-data?id=${chapterId}`;
        } else if (mode === 'review') {
            apiUrl = `/.netlify/functions/get-question-by-id?id=${urlParams.get('questionId')}`;
        } else {
            console.log("⚠️ 未检测到 homework 标签，尝试执行普通章节逻辑");
            if (!chapterId) { 
                console.error("❌ 错误：既没有 source 也没有 chapterId");
                alert("未指定章节ID"); 
                return; 
            }
        ...
        console.log("📥 后端返回原始数据长度:", data ? data.length : 0);

        // 🔍 诊断 3：检查 shifouzuoguo 过滤逻辑
        // 注意：我们直接根据你提供的字段名进行过滤
        questionSet = Array.isArray(data) ? data.filter(q => {
            const isDone = q.shifouzuoguo === true;
            console.log(`题目 ID ${q.id} 的 shifouzuoguo 状态:`, q.shifouzuoguo);
            return !isDone; // 只保留未做过的
        }) : [];

        console.log("🎯 过滤后待显示题目数:", questionSet.length);

        if (questionSet.length === 0) {
            questionTextEl.textContent = '🎉 今天的作业已经全部完成了！';
